...
class _UserRepository extends BaseRepository
  ...
  Future<List<int>> insert(List<UserInsertRequest> requests) async {
    ...
    var rows = await db.query(
      'INSERT INTO "users" ( "username", "email", "password", "phone", "avatar", "first_name", "last_name", "code", "code_life" )\n'
      'VALUES ${requests.map((r) => '( ${values.add(r.username)}:text, ${values.add(r.email)}:text, ${values.add(r.password)}:text, ${values.add(r.phone)}:text, ${values.add(r.avatar)}:text, ${values.add(r.firstName)}:text, ${values.add(r.lastName)}:text, ${values.add(r.code)}:text, ${values.add(r.codeLife)}:text )').join(', ')}\n'
      'RETURNING "id"',
      values.values,
    );
  ...
  Future<void> update(List<UserUpdateRequest> requests) async {
    ...
    await db.query(
      ...
      'FROM ( VALUES ${requests.map((r) => '( ${values.add(r.id)}:int8::int8, ${values.add(r.username)}:text::text, ${values.add(r.email)}:text::text, ${values.add(r.password)}:text::text, ${values.add(r.phone)}:text::text, ${values.add(r.avatar)}:text::text, ${values.add(r.firstName)}:text::text, ${values.add(r.lastName)}:text::text, ${values.add(r.code)}:text::text, ${values.add(r.codeLife)}:text::text )').join(', ')} )\n'
      'AS UPDATED("id", "username", "email", "password", "phone", "avatar", "first_name", "last_name", "code", "code_life")\n'
